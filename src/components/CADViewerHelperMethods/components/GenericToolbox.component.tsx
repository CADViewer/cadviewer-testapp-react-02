...
function lock_single() {
	...
	cadviewer.cvjs_showOnlyObjectInSpaceObjectGroup((door_id as string), "locked", "id");
}

function open_single() {
	var door_id = JQ("#door_id").val();
	cadviewer.cvjs_showOnlyObjectInSpaceObjectGroup((door_id as string), "open", "id");
}

function close_single() {
	var door_id = JQ("#door_id").val();
	cadviewer.cvjs_showOnlyObjectInSpaceObjectGroup((door_id as string), "closed", "id");
}

function hide_space_object() {
	var door_id = JQ("#door_id").val();
	cadviewer.cvjs_hideSpaceObjectID((door_id as string));
}

function open_node() {
	var door_id = JQ("#door_id").val();
	cadviewer.cvjs_open_StickyNote((door_id as string));
}

function close_node() {
	var door_id = JQ("#door_id").val();
	cadviewer.cvjs_close_StickyNote((door_id as string));
}

function hide_node() {
	var door_id = JQ("#door_id").val();
	cadviewer.cvjs_hide_StickyNote((door_id as string));
}

function show_node() {
	var door_id = JQ("#door_id").val();
	cadviewer.cvjs_show_StickyNote((door_id as string));
}

function hide_object_in_group() {
	var id = JQ("#copy_new_id").val();
	var node = cadviewer.cvjs_getSpaceObjectNodefromId((id as string));
	var objectTag = JQ("#group_2_subid").val();
	cadviewer.cvjs_hideObjectInSpaceObjectGroup((id as string), node, (objectTag as string));
}
...
function show_object_in_group() {
	...
	var node = cadviewer.cvjs_getSpaceObjectNodefromId((id as string));
	var objectTag = JQ("#group_2_subid").val();
	cadviewer.cvjs_showObjectInSpaceObjectGroup((id as string), node, (objectTag as string));
}
...
function display_all_objects() {
	...
	for (var spc in allSpaceObjects.SpaceObjects) {
		...
		myString +=
			"(" +
			allSpaceObjects.SpaceObjects[spc].id +
			", " +
			allSpaceObjects.SpaceObjects[spc].area +
			")";
	}
...
function customAddTextToSpaces() {
	...
	for (var spc in spaceObjectIds) {
		...
		if (i % 3 == 0) {
			...
			cadviewer.cvjs_AddTextOnSpaceObject(
				CV.textLayer1,
				spaceObjectIds[spc],
				0.05,
				textString,
...
function highlight_all_spaces() {
	selectedColor = (JQ("#input_color").val() as string);
...
function highlight_space_type() {
	...
	var spaceObjectIds = cadviewer.cvjs_getSpaceObjectIdList();
	for (var spc in spaceObjectIds) {
		var spaceType = cadviewer.cvjs_getSpaceObjectTypefromId(
			spaceObjectIds[spc]
		);
		var vqid = spaceObjectIds[spc];
		console.log(
			spc + "  " + vqid + "  " + spaceType + "  " + spaceType.indexOf("Device")
		);
		if (spaceType.indexOf((type as string)) == 0)
...
function highlight_space_id() {
	selectedColor = (JQ("#input_color").val() as string);
	var spaceid = (JQ("#image_ID").val() as string);
	var colortype = {
		fill: selectedColor,
		"fill-opacity": "0.8",
		stroke: selectedColor,
	...
	for (var spc in spaceObjectIds) {
		console.log(
			spaceid +
				...
				spaceObjectIds[spc] +
				" " +
				(spaceid.indexOf(spaceObjectIds[spc]) == 0) +
				" " +
				(spaceid.length == spaceObjectIds[spc].length)
		);
		if (
			spaceid.indexOf(spaceObjectIds[spc]) == 0 &&
			spaceid.length == spaceObjectIds[spc].length
...
function insert_from_type_id_image() {
	var loadSpaceImage_Location =
		"http://localhost:3005/content/drawings/svg/" +
		JQ("#image_sensor_location").val();
	var loadSpaceImage_ID = (JQ("#image_ID").val() as string);
	var loadSpaceImage_Type = (JQ("#image_Type").val() as string);
	var loadSpaceImage_Layer = "cvjs_SpaceLayer";

	cadviewer.cvjs_setImageSpaceObjectParameters(
...
function cadviewerCanvasMethod01() {
	cadviewer.cvjs_executeCustomCanvasMethod_drag(
		generic_start_method_01,
		generic_stop_method_01,
		generic_move_method_01,
		""
	);
}

function cadviewerCanvasMethod02() {
	cadviewer.cvjs_executeCustomCanvasMethod_click(
		generic_mousemove_method_01,
		generic_mousedown_method_01,
		generic_mouseup_method_01,
		generic_dblclick_method_01,
		""
	);
}
...
function cadviewerCanvasMethod03() {
	cadviewer.cvjs_executeCustomCanvasMethod_click(
		generic_make_rect_mousemove_method,
		generic_make_rect_mousedown_method,
		generic_make_rect_mouseup_method,
		"",
		generic_make_rect_init_method
	);
}

function cadviewerCanvasMethod04() {
	cadviewer.cvjs_executeCustomCanvasMethod_drag(
		generic_drag_rect_start,
		generic_drag_rect_stop,
		generic_drag_rect_move,
		""
	);
}
...
function cadviewerCanvasMethod05() {
	cadviewer.cvjs_executeCustomCanvasMethod_drag(
		select_drag_rect_start,
		select_drag_rect_stop,
		select_drag_rect_move,
		""
	);
}
...
function cadviewerCanvasMethod06() {
	cadviewer.cvjs_executeCustomCanvasMethod_click(
		generic_make_rect_arrow_mousemove_method,
		generic_make_rect_arrow_mousedown_method,
		generic_make_rect_arrow_mouseup_method,
		generic_make_rect_arrow_dblclick_method,
		generic_make_rect_arrow_init_method
	);
}

function cadviewerCanvasMethod07() {
	cadviewer.cvjs_executeCustomCanvasMethod_click(
		generic_handles_mousemove_method_01,
		generic_handles_mousedown_method_01,
		generic_handles_mouseup_method_01,
		generic_handles_dblclick_method_01,
		generic_handles_init_method_01
	);
}
 ...
 */
var generic_move_method_01 = function (dx: number, dy : number, x : number, y : number) {
	var svg_x = cadviewer.cvjs_SVG_x(x);
	var svg_y = cadviewer.cvjs_SVG_y(y);
	console.log(
		"generic_move_method_01: dx=" +
			dx +
			" dy=" +
			dy +
			" x=" +
...
var generic_mousemove_method_01 = function (e :any, x : number, y : number) {
	var svg_x = cadviewer.cvjs_SVG_x(x);
	var svg_y = cadviewer.cvjs_SVG_y(y);

	//cadviewer.cvjs_removeCustomCanvasMethod();
	console.log(
		"generic_mousemove_method_01: x=" +
			x +
			" y=" +
			y +
			" svg_x=" +
...
var generic_mousedown_method_01 = function (e :any, x : number, y : number) {
	var svg_x = cadviewer.cvjs_SVG_x(x);
	var svg_y = cadviewer.cvjs_SVG_y(y);

	//cadviewer.cvjs_removeCustomCanvasMethod();
	console.log(
		"generic_mousedown_method_01: x=" +
			x +
			" y=" +
			y +
			" svg_x=" +
...
var generic_mouseup_method_01 = function (e :any, x :number, y :number) {
	var svg_x = cadviewer.cvjs_SVG_x(x);
	var svg_y = cadviewer.cvjs_SVG_y(y);

	console.log(
		"generic_mouseup_method_01: x=" +
			x +
			" y=" +
			y +
			" svg_x=" +
...
var generic_dblclick_method_01 = function (e :any, x :number, y :number) {
	cadviewer.cvjs_removeCustomCanvasMethod();

	var svg_x = cadviewer.cvjs_SVG_x(x);
	var svg_y = cadviewer.cvjs_SVG_y(y);
	console.log(
		"REMOVE: generic_dblclick_method_01: x=" +
			x +
			" y=" +
			y +
			" svg_x=" +
...
var generic_make_rect_mousedown_method = function (e :any, x :number, y :number) {
	try {
		console.log("generic_make_rect_mousedown_method");

		if (!generic_canvas_flag_first_click_rectangle) {
			var tPath_r = "M" + 0 + "," + 0;
			cvjs_RubberBand = cadviewer
				.cvjs_makeGraphicsObjectOnCanvas("Path", tPath_r)
				.attr({ stroke: "#b00000", fill: "none", "stroke-width": 0.5 });
			generic_canvas_flag_first_click_rectangle = true;
			generic_canvas_flag_rectangle = false;
			console.log("rubberband");
		} else {
			console.log(
				generic_canvas_flag_first_click_rectangle +
					"  " +
					generic_canvas_flag_rectangle
			);
...
var generic_make_rect_mousemove_method = function (e :any, x :number, y :number) {
	try {
		if (generic_canvas_flag_first_click_rectangle) {
...
var generic_make_rect_stop_method = function () {
	...
	var Node = cadviewer.cvjs_currentMaxSpaceNodeId();
    var Node_id = parseInt(Node);
    if (isNaN(Node_id)){
        Node_id = 1;
    }
	...
	cadviewer.cvjs_createSpaceObjectGroup(
	...
	cadviewer.cvjs_addGraphicsToSpaceObjectGroup(
		currentNode_underbar,
		grayBox,
		"rect01"
	);
 ...
 * Template move drag method for custom canvas
 */
var generic_drag_rect_move = function (dx :number, dy :number, x :number, y : number) {
	try {
		if (generic_canvas_flag_first_click_rectangle) {
			cadviewer.cvjs_customCanvasMethod_globalScale();
			if (!generic_canvas_flag_rectangle) {
...
var select_drag_rect_move = function (dx : number, dy : number, x : number, y : number) {
	try {
		if (generic_canvas_flag_first_click_rectangle) {
			cadviewer.cvjs_customCanvasMethod_globalScale();
			if (!generic_canvas_flag_rectangle) {
...
var select_drag_rect_stop = function () {
	...
	for (var spc in spaceObjectIds) {
		...
		overlap = cadviewer.cvjs_rect_doOverlap(
			cvjs_firstX,
			cvjs_firstY,
			cvjs_lastX,
			cvjs_lastY,
		...
		if (overlap) selected_list += spaceObjectIds[spc] + ";";
	}
...
var generic_make_rect_arrow_mousedown_method = function (e :any, x :number, y :number) {
	try {
		if (
			operation_type.indexOf("arrow") == 0 &&
			logic_flags[0] &&
			logic_flags[1]
		) {
...
var generic_make_rect_arrow_mousemove_method = function (e :any, x :number, y :number) {
	try {
		if (operation_type.indexOf("box") == 0 && logic_flags[0]) {
			cadviewer.cvjs_customCanvasMethod_globalScale();
			if (!logic_flags[1]) {
...
var generic_make_rect_arrow_stop_method1 = function () {
	...
	randomNr = Math.floor(Math.random() * Math.floor(100000));
	window.alert("randomNr " + randomNr);
	...
	var myText = cadviewer
		.cvjs_makeTextObjectOnCanvas(
			cvjs_firstX + (cvjs_lastX - cvjs_firstX) / 10.0,
			cvjs_lastY - (cvjs_lastY - cvjs_firstY) / 5.0,
			"API" + randomNr
		)
		.attr({
			fill: "#000",
			"fill-opacity": "1",
			stroke: "#000",
			"font-size": Math.abs(cvjs_lastY - cvjs_firstY) / 2.0,
		});
	cadviewer.cvjs_addGraphicsToSpaceObjectGroup(
		currentNode_underbar,
		myText,
...
var generic_make_rect_arrow_stop_method2 = function () {
	...
	// create an arrow
	var angleInDegrees =
		(Math.atan2(cvjs_lastY - cvjs_firstY, cvjs_lastX - cvjs_firstX) / Math.PI) *
		180.0;
	...
	var Ttrans =
		...
		(angleInDegrees - 270) +
		"T" +
		(cvjs_lastX + mycos * scaleTriangle) +
		"," +
		cvjs_lastY;
	var Triangle = cadviewer.cvjs_makeGraphicsObjectOnCanvas(
		"Polyline",
		triangle_design
	);
	Triangle.attr({
	...
	try {
//		this.cvjs_graphicalObjectOnChange(
            CV.cvjs_graphicalObjectOnChange(
			"Create",
			"CustomObject",
			"API" + randomNr
		);
	} catch (err) {
		console.log("cvjs_graphicalObjectOnChange: arrow method:" + err);
	}
	cadviewer.cvjs_removeCustomCanvasMethod();
...
var generic_handles_mousemove_method_01 = function (e :any, x :number, y :number) {
	var svg_x = cadviewer.cvjs_SVG_x(x);
	var svg_y = cadviewer.cvjs_SVG_y(y);
...
var generic_handles_mousedown_method_01 = function (e :any, x :number, y :number) {
	var svg_x = cadviewer.cvjs_SVG_x(x);
	var svg_y = cadviewer.cvjs_SVG_y(y);
...
var generic_handles_mouseup_method_01 = function (e :any, x :number, y :number) {
	var svg_x = cadviewer.cvjs_SVG_x(x);
	var svg_y = cadviewer.cvjs_SVG_y(y);
...
var generic_handles_dblclick_method_01 = function (e :any, x :number, y :number) {
	cadviewer.cvjs_removeCustomCanvasMethod();
	...
	for (var spc in selected_handles) {
		...
		cadviewer.cvjs_mouseout_handleObjectStyles(
			selected_handles[spc].id,
			selected_handles[spc].handle
		);
	}
...
function checknumberoftypes() {
	...
	window.alert(
		"Number of " + mytype + " types=" + myspaces.SpaceObjects.length
	);
}
...
{/*			<input type="text" id="rotate_id" value="" className="block w-full rounded-md border-0 bg-black/5 pl-2 py-1.5 text-black shadow-sm ring-1 ring-inset ring-black/10 focus:ring-2 focus:ring-inset focus:ring-primary-500 sm:text-sm sm:leading-6"/>
*/}
		...
		{/*}
			...
			New Space Type/ID/Image:&nbsp; <button className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2" onClick="insert_from_type_id_image();">New Space Object</button>&nbsp;&nbsp; Update Group ID: &nbsp; <button className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2" onClick="update_group_with_path();">Add Path Object to Group</button>&nbsp; 
			<br/>Second group ID: &nbsp; &nbsp;<input type="text" className="shadow appearance-none border rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="group_2" defaultValue="NODE_xx" />&nbsp;<input type="text" className="shadow appearance-none border rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="group_2_subid" defaultValue="id_01" />&nbsp;<button className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2" onClick="update_group_with_group();">Add Group to Group</button>&nbsp; &nbsp; <button className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2" onClick="hide_object_in_group();">Hide Object In Group</button>&nbsp;&nbsp;  <button className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2" onClick="show_object_in_group();">Show Object In Group</button>&nbsp;
		*/}
		{/* <canvas id="dummy" width="5" height="21"></canvas>
			...
			<input
				type="text"
				className="shadow appearance-none border rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
				id="image_sensor_location"
				defaultValue="wifi_25.svg"
			/>
			<strong>Create:</strong>&nbsp;{" "}
			<button
				className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2"
				onClick={insert_from_type_id_image}
			>
				New Space Object
			</button>
			<br />
			<canvas id="dummy" width="5" height="22"></canvas>
			<b>Highlight Spaces based on Color:&nbsp; </b>{" "}
			<input
				type="text"
				className="shadow appearance-none border rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
				id="input_color"
			...
			<button
				className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2"
				onClick={highlight_all_spaces}
			>
				Spaces
			</button>
			<button
				className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2"
				onClick={highlight_all_borders}
			>
				Borders
			</button>
			<button
				className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2"
				onClick={highlight_space_type}
			>
				Space Type
			</button>
			<button
				className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2"
				onClick={highlight_space_id}
			>
				Space ID
			</button>
			<button
				className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2"
				onClick={clear_space_highlight}
			>
				Clear All
			</button>
			<button
				className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2"
				onClick={display_all_objects}
			>
				All:(id,area)
			</button>
			<button
				className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2"
				onClick={customAddTextToSpaces}
			>
				Text on Spaces
			</button>
			<br /> */}
			{/* <canvas id="dummy" width="5" height="22"></canvas>
				<strong>Custom Interactive Canvas Samples:&nbsp;</strong>
				<canvas id="dummy" width="10" height="10"></canvas>
				<button
					className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2"
					onClick={cadviewerCanvasMethod01}
				>
					Canvas-DRAG (console)
				</button>
				<button
					className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2"
					onClick={cadviewerCanvasMethod02}
				>
					Canvas-CLICK (console)
				</button>
				<button
					className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2"
					onClick={cadviewerCanvasMethod03}
				>
					Make Rect -CLICK
				</button>
				<button
					className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2"
					onClick={cadviewerCanvasMethod04}
				>
					Make Rect -DRAG
				</button>
				<button
					className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2"
					onClick={cadviewerCanvasMethod05}
				>
					Select Spaces -DRAG (rl/tl in ax2023)
				</button>
				<button
					className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2"
					onClick={cadviewerCanvasMethod06}
				>
					Make Box/Arrow Canvas-CLICK
				</button>
				<button
					className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2"
					onClick={cadviewerCanvasMethod07}
				>
					Select Handles -CLICK (DblClick End) (hlall in ax2023)
				</button>
				<button
					className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2"
					onClick={loadprocessedsvg}
				>
					Load processed SVG
				</button>
				<button
					className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2"
					onClick={checknumberoftypes}
				>
					Check number of types
				</button> */}
			{/*
				<br/><b><i>IOT commands:</i>&nbsp; </b> 
				&nbsp;&nbsp;&nbsp;&nbsp;  <button className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2" onClick={copy_group_object}><i>Copy Group</i></button>&nbsp;<input type="text" className="shadow appearance-none border rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="copy_org_id" defaultValue="orgid" />&nbsp;<input type="text" className="shadow appearance-none border rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="copy_new_id" defaultValue="newid" /><button className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2" onClick={hide_object_in_group}>Hide Subgroup In Group</button>&nbsp;&nbsp;  <button className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2" onClick={show_object_in_group}>Show Subgroup In Group</button>&nbsp;
				<br/><i>Group 1:</i> &nbsp; &nbsp;<input type="text" className="shadow appearance-none border rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="group_1" defaultValue="NODE_xx" />  <i>Group 2:</i> &nbsp; &nbsp;<input type="text" className="shadow appearance-none border rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="group_2" defaultValue="NODE_yy" />&nbsp;<button className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2" onClick={update_group_with_group}><i>Add Group to Group</i></button>&nbsp;&nbsp;Subgroup ID:&nbsp;<input type="text" className="shadow appearance-none border rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="group_2_subid" defaultValue="id_01" />&nbsp; &nbsp; 
			*/}
			{/* <br />
				<canvas id="dummy" width="5" height="22"></canvas>
				<b>Place out Door Objects:&nbsp;</b>
				<button
					className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2"
					onClick={close_all}
				>
				...
				<button
					className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2"
					onClick={retrieve_redlines_stickynotes}
				>
					Retrieve Deleted Redlines
				</button>
				&nbsp;
				<button
					className="bg-primary-500 hover:bg-primary-700 text-white font-semibold py-1 px-3 rounded m-2"
					onClick={hide_space_object}
				>
					Hide Space Object
				</button>
				&nbsp;&nbsp;
