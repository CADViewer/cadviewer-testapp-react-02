...
export const isBranchNode = (data: INode[], i: NodeId) => {
  ...
  // @ts-ignore
  return !!node.children?.length || node.own === true || node.isFolder === true;
};
...
export const flattenTree = <M extends IFlatMetadata>(
  ...
  const flattenTreeHelper = (tree: ITreeNode<M>, parent: NodeId | null) => {
    const node: INode<M> = {
      ...
      own: tree.own,
      children: [],
      ...((tree.isBranch || tree.isFolder) && { isBranch: tree.isBranch || tree.isFolder }),
      parent,
      // @ts-ignore
      metadata: tree.metadata ? { ...tree.metadata } : {
				path: tree.path,
